...
urls = dict(
    station_obs=[
        "https://pavics.ouranos.ca/twitcher/ows/proxy/thredds/catalog/datasets/station_obs/catalog.xml"
    ],
    gridded_obs=[
        "https://pavics.ouranos.ca/twitcher/ows/proxy/thredds/catalog/datasets/gridded_obs/catalog.xml"
    ],
    reanalyses=[
        "https://pavics.ouranos.ca/twitcher/ows/proxy/thredds/catalog/datasets/reanalyses/catalog.xml"
    ],
    forecasts=[
        "https://pavics.ouranos.ca/twitcher/ows/proxy/thredds/catalog/datasets/forecasts/catalog.xml"
    ],
    simulations=[
        "https://esgf.ouranos.ca/thredds/catalog/aggregations/CORDEX-CMIP6/catalog.xml",
...
for key, urls in urls.items():
    ...
    for url in urls:
        ...
        for dd in walk(cat, depth=10):
            ...
            if (
                "OURANOS_CRCM5" in ncml
                and "https://pavics.ouranos.ca/twitcher/ows/proxy/thredds/catalog/datasets/simulations/"
                in url
            ):
                continue
            ...
            if "ESPO-R" not in ncml:
                ...
                for ntry in range(0, 5):
                    try:
                        ds = xr.open_dataset(
                            dd.access_urls[dap_key], chunks=dict(time=100)
                        )
                        break
                    except:
                        ntry -= 1
