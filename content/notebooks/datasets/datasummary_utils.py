...
def correct_institutes(df):
    ...
    inst = [
        (
            o.replace(", Victoria, BC, www.pacificclimate.org", "")
            if "PCIC" in o
            else o
        )
    ...
    inst = [
        (
            o.replace("Canadian Centre for Climate Services", "Canadian Centre for Climate Services : Climatedata.ca")
...
def correct_titles(df):
    ...
    titles = [
        o.replace("The ClimEx", "The ClimEx") if "ClimEx" in o else o
        for o in titles
    ]
    ...
    titles = [
        o.replace("PINS v1", "PINS v1") if "PINS v1" in o else o
        for o in titles
    ]
    df["title"] = titles
    return df
...
def create_map(dfin, overwrite=False):
    ...
    if not outpng.exists() or overwrite:
        ...
        if len(data.dims) == 1:
            ...
            data.plot(ax=ax, x="lon", y="lat", cmap ="RdBu_r", transform=ccrs.PlateCarree())
        ax.set_title('')
...
def create_variable_tab(dfin, lang='en', overwrite=False):
    ...
    outcsv= Path('dataset_variable_details', f"{dfin['title'].values[0].replace('/','_')}_{lang}.csv")
    ...
    if not outcsv.exists() or overwrite:
        ...
        for index, row in tqdm(dfin.iterrows(), total=len(dfin)):
            ...
            for vv in ds_tmp.data_vars:
                #if "time" not in ds_tmp[vv].dims:
                if not {k for k in chunks.keys() if k != 'time'}.issubset(ds_tmp[vv].dims):
                    ds_tmp = ds_tmp.assign_coords({vv: ds_tmp[vv]})
            ...
            for vv in [v for v in ds_tmp.data_vars if "_consensus" not in v]:
                ...
                if vv_out not in vars_dict:
                    ...
                    if freqout not in vars_dict[vv_out][tmp_grp]:
                        vars_dict[vv_out][tmp_grp] = vars_dict[vv_out][tmp_grp] + '; ' + freqout
        ...
        vars_table = vars_table[[desc, units, tmp_grp, cell_name,]]
...
def create_summary_tabs(dfin, lang='en', overwrite=False):
    ...
    tabs = pn.Tabs((summary_fields[lang]['title'], summ), 
                   ('Variables', var1), 
